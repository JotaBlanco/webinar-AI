...
5. **Write strict JSON to `module-1/_grade/canonical/agent-02_final-model.json`.** No prose around it, no markdown fence.
...
After writing, return one short sentence: `Canonical eval for agent-02_final-model: status=<ok|failed>`. Nothing else.
