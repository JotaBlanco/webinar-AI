"""Fit M5 (long-lat coupled single-track + friction circle) on the frozen train split.

Fits {C_alpha_f, C_alpha_r, I_z, mu_f, mu_r, drive_split_front,
brake_split_front} per platform against pooled yaw RMSE on train,
evaluates on dev for the gap report, writes coeffs.json.
...
from _shared.physics_core import prior  # noqa: E402
...
def initial_coeffs(platforms) -> dict[str, dict[str, float]]:
    ...
    for plat in platforms:
        ...
        out[plat] = {
            ...
            "mu_f":              p["mu"],
            "mu_r":              p["mu"],
            "drive_split_front": 0.5,
            "brake_split_front": 0.6,
        }
    return out
...
def default_bounds(platforms) -> dict[str, dict[str, tuple[float, float]]]:
    ...
    for plat in platforms:
        p = prior(plat)
        out[plat] = {
            "C_alpha_f":         (0.3 * p["C_alpha_f"], 3.0 * p["C_alpha_f"]),
            "C_alpha_r":         (0.3 * p["C_alpha_r"], 3.0 * p["C_alpha_r"]),
            "I_z":               (0.5 * p["I_z"],       2.0 * p["I_z"]),
            "mu_f":              (0.7, 1.2),
            "mu_r":              (0.7, 1.2),
            "drive_split_front": (0.2, 0.8),
            "brake_split_front": (0.4, 0.8),
        }
    return out
...
def main() -> int:
    ...
    p.add_argument("--with-bounds", action="store_true",
                   help="Use L-BFGS-B with carParams-derived bounds (default: Nelder-Mead).")
    args = p.parse_args()
    ...
    print(f"M5 fit — {len(train)} train segments, {len(dev)} dev, "
