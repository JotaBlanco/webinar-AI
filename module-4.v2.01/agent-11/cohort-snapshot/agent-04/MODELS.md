...
- status: shelved
- pooled-yaw-rmse-dev: 0.00919 (priors only — fit did not converge inside wall-clock budget under heavy cohort CPU contention)
- pooled-cte-rmse-dev: 116.89
- vs-v1: yaw -69%, CTE -124% at priors (does not reflect a fit)
- vs-parent: n/a (root of dynamics branch)
- gate: fail (rung-climb attempted but unfit; dropped because parallel-cohort fit.py runs starved the L-BFGS-B optimisation)
- next: rerun fit on a quiet machine; bound `C_α` ratio against `m·g·l_r/L` to cut to one degree of freedom per axle
- notes: canonical rung-1 climb. Two-state ODE [β, ψ̇], RK4. Attempt logged for cohort tracking; not shipped.
...
- structure: differs-from-v1
- status: kept
- pooled-yaw-rmse-dev: 0.005636 (σ grid-fit: F150=0.3, MachE=0.5, Ioniq=0.3)
- pooled-cte-rmse-dev: 52.15
- vs-v1: yaw -3.8%, CTE +0.13% (near-tie; CTE marginally improved, yaw marginally worse)
- vs-parent: n/a
- gate: pass
- next: shelve as orthogonal — does not strictly dominate V1 on either KPI
- notes: distance-domain relaxation. σ ranged 0.3–0.5 m per platform (well within physics-priors range). Did not improve enough to ship.
...
- vs-v1: yaw -0.0%, CTE -0.0% (this IS the V1 shape; the m3.v3 leader coeffs are constants of record)
- vs-parent: n/a
- gate: pass
- next: shipped at final-model/predict.py
- notes: V1 kinematic + understeer + first-order lag + per-segment δ₀. Cohort leader since m3.v2. Shipped because no candidate strictly dominated on either KPI inside budget.
