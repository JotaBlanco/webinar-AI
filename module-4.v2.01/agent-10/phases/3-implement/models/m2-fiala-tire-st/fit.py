"""Fit M2 (Fiala-tire single-track) coefficients on the frozen train split.

Fits {C_alpha_f, C_alpha_r, I_z, mu_f, mu_r} per platform against pooled
yaw RMSE on train, evaluates on dev for the gap report, writes coeffs.json.
...
def main() -> int:
    ...
    print(f"M2 fit — {len(train)} train segments, {len(dev)} dev, "
