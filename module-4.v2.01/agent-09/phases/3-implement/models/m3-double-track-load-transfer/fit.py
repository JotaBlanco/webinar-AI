"""Fit M3 (double-track + lateral load transfer) coefficients on the frozen train split.

Fits {C_alpha_f, C_alpha_r, I_z, mu_f, mu_r, h_cg} per platform against
pooled yaw RMSE on train, evaluates on dev for the gap report, writes
coeffs.json.

Usage:
    python fit.py                   # fits all platforms with truth
    python fit.py --platforms FORD_F_150_LIGHTNING_MK1
...
def main() -> int:
    ...
    print(f"M3 fit — {len(train)} train segments, {len(dev)} dev, "
