"""Fit M1 (linear dynamic single-track) coefficients on the frozen train split.

Fits {C_alpha_f, C_alpha_r, I_z} per platform against pooled yaw RMSE on
train, evaluates on dev for the gap report, writes coeffs.json.
...
def main() -> int:
    ...
    print(f"M1 fit — {len(train)} train segments, {len(dev)} dev, "
