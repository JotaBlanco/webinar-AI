...
# Pull sibling skill dirs (hyphenated, can't be package-imported) onto sys.path
# so we can load their entry points. The template root itself goes on sys.path
# so `from code import v1_baseline` works — same convention as iterate.py uses.
# This is the m4.v1.01 normalisation: never `from v1_baseline import …` (would
# need code/ to be on sys.path), always `from code import v1_baseline` (uses
# the code/ package).
for _p in (
    ...
    AGENT_DIR / "_shared",
    AGENT_DIR,  # template root → makes `code` importable as a package
):
...
def _load_v1():
    # Same convention as skills/iterate/iterate.py: code/ is a package
    # (it has __init__.py); template root is on sys.path.
    from code import v1_baseline  # type: ignore
    return v1_baseline.predict_v1
