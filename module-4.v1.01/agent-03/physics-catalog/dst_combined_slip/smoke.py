"""dst_combined_slip smoke — synthetic brake-into-corner case."""
...
def _segment(*, hard_brake_corner: bool) -> pd.DataFrame:
    n, dt = 600, 0.01
    t = np.arange(n) * dt
    delta = 0.08 * np.sin(2 * np.pi * 0.3 * t)  # sustained cornering
    v = 15.0 + 0.05 * t
    # Hard braking burst in the middle of the corner.
    a_long = np.where((hard_brake_corner) & (t > 2.0) & (t < 4.0), -6.0, 0.05)
...
def main() -> int:
    ...
    for platform in PLATFORM_PRIORS:
        for mode in ("cruise_corner", "brake_corner"):
            seg = _segment(hard_brake_corner=(mode == "brake_corner"))
            try:
