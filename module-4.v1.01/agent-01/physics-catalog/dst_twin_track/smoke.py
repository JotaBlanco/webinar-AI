"""dst_twin_track smoke — synthetic cornering segment."""
...
def _segment(*, hard_corner: bool) -> pd.DataFrame:
    n, dt = 600, 0.01
    t = np.arange(n) * dt
    amp = 0.10 if hard_corner else 0.04
    delta = amp * np.sin(2 * np.pi * 0.3 * t)
    v = 18.0 if hard_corner else 10.0
    v_arr = np.full(n, v)
    psi_v0 = v_arr * np.tan(delta) / 3.0
    return pd.DataFrame({
        "t_s": t, "delta_wheel_deg": np.degrees(delta) * 16.0,
        "delta_road_rad": delta, "v_mps": v_arr,
...
def main() -> int:
    ...
    for platform in PLATFORM_PRIORS:
        for mode in ("cruise", "hard_corner"):
            seg = _segment(hard_corner=(mode == "hard_corner"))
            try:
