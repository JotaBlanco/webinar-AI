...
def critique(gate_output: dict, residual_verdict: str, cohort_hint: bool = True) -> dict:
    ...
    # 'unknown' = assessment.md was missing when iterate ran. Different
    # situation than 'noise_floor' — agent should fix the diagnostic step
    # before routing. Avoids the prior failure mode where 'noise_floor'
    # and 'unknown' both routed to try_residual_learner.
    if residual_verdict == "unknown":
        return Route(
            name="run_assessment_first",
            confidence="high",
            rationale="assessment.md was missing when iterate ran. Run skills/assess-candidate-model/ on this bundle, then re-iterate.",
            cohort_precedent=None,
        ).__dict__

    reasons = set(gate.get("reasons", []))

    # m4.v1.01 — cohort §6 + §9 inline prior. Fires when the iterate gate
    # already flagged a bias term without route-CV σ. The route's job is to
    # route the agent to *do the route-CV now*, before the bias is shipped.
    # This was a warning in m4.v1 (sat only in references/); agent-07
    # negotiated it away under time pressure. Now it's emitted as a
    # next_move string the agent sees after every iterate call.
    if any(r.startswith("bias_without_route_cv") for r in reasons):
        return Route(
            name="add_route_cv_for_bias",
            confidence="high",
            rationale=(
                "Candidate ships a per-platform bias term but coeffs.json has no "
                "route_cv_sigma sibling. Cohort §6 + §9: subset fits flip the "
                "Lightning bias sign — the m4.v1 agent-07 mode (+44.6% yaw, "
                "vs cohort median +56.2%). Re-fit the bias under "
                "skills/score-model/cv.py:score_cv with route-grouped folds; "
                "write the σ back into coeffs.json under each platform."
            ),
            cohort_precedent=f"{COHORT_REFS} §6, §9",
        ).__dict__
