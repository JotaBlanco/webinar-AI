...
def _run_gate(cand_cv: dict, vs_parent: dict, fit_diagnostics: dict | None,
              bias_gate: tuple[bool, str] | None = None) -> GateResult:
    reasons: list[str] = []
    if fit_diagnostics:
    ...
    # m4.v1.01 — hard refuse on per-platform bias terms without route-CV σ.
    # Cohort §6 + §9; see _shared/gates.py:check_bias_without_route_cv.
    bias_fail = False
    if bias_gate is not None and not bias_gate[0]:
        reasons.append(f"bias_without_route_cv: {bias_gate[1]}")
        bias_fail = True
    status = (
        "fail" if (bias_fail or any(r.startswith("fit_non_convergence") for r in reasons))
...
def _check_bias_gate(model_dir: Path, template_root: Path) -> tuple[bool, str]:
    """Load _shared/gates.py and run check_bias_without_route_cv on coeffs.json.

    Safe under environments where _shared/ is absent (returns vacuous pass).
    """
    coeffs_path = model_dir / "coeffs.json"
    gates_path = template_root / "_shared" / "gates.py"
    if not gates_path.exists():
        return True, "_shared/gates.py absent; bias gate skipped"
    try:
        spec = importlib.util.spec_from_file_location("_m4v101_gates", str(gates_path))
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        return mod.check_bias_without_route_cv(coeffs_path)
    except Exception as e:
        return True, f"bias gate skipped ({type(e).__name__}: {e})"
...
def iterate(model_dir: str | Path, parent: str | None = None, rung: str | None = None) -> dict:
    ...
    # m4.v1.01 — bias-without-route-CV gate. Pull from _shared/gates.py.
    bias_gate = _check_bias_gate(model_dir, template_root)

    gate = _run_gate(cand_cv, vs_parent, fit_diagnostics, bias_gate=bias_gate)
