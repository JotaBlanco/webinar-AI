...
if ! command -v yq >/dev/null 2>&1; then
    cat >&2 <<'EOF'
yq not found. Three options:
  1. Install yq:
       macOS:  brew install yq
       Linux:  see https://github.com/mikefarah/yq#install
       (Or: pip install yq for the Python build — works everywhere.)
  2. Skip the shell driver entirely and dispatch from inside a Claude
     Code session — open launch-rungs/orchestrate.md and follow its
     in-process Task-tool protocol. Same fan-out, no yq required.
  3. Adapt manually: launch-rungs/manifest.yaml is a small YAML file
     listing 6 subagents with prompt_hint, budget_minutes, and
     catalog_starter fields. Any harness can read it via python -c
     "import yaml; print(yaml.safe_load(open('manifest.yaml')))".
EOF
    exit 1
fi
if [ "$DRY" -eq 0 ]; then
