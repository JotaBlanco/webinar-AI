"""Refit V1 coeffs on ALL segments per platform — should give tighter coeffs."""
import sys
...
def cost_for_platform(theta, segs_cached, use_per_segment_delta0):
    ...
    tot_sq = 0.0
    tot_n = 0
    for cache in segs_cached:
        d0 = cache["delta0"] if use_per_segment_delta0 else delta0_global
        sim = cache["df"]
...
def fit_platform(plat, use_per_segment_delta0, init):
    ...
    def obj(theta):
        return cost_for_platform(theta, cached, use_per_segment_delta0)

    x0 = np.array(init)
    res = minimize(obj, x0, method="Nelder-Mead",
                   options={"xatol": 1e-6, "fatol": 1e-11, "maxiter": 800, "disp": False})
...
import json
out = ROOT / "out" / "fitted_coeffs_full.json"
