"""Fit V3: V2 + cubic delta term to address sign-asymmetry.

    yaw = v * (delta_eff + alpha3 * delta_eff^3) / (L + K_us*v^2) + bias

delta_eff = delta_road + tau * d_delta_dt
"""
import json
import sys
...
def predict_factory_v3(platform, coeffs):
...
def main():
    ...
    # Seed from V2 fitted coeffs
    v2 = json.loads((ROOT / "out" / "coeffs_v2.json").read_text())
    init = {plat: {
        "L_eff":  v2[plat]["L_eff"],
        "K_us":   v2[plat]["K_us"],
        "tau":    v2[plat]["tau"],
    ...
    coeffs["TESLA_MODEL_3"] = {"L_eff": L_PRIOR["TESLA_MODEL_3"], "K_us": 0.0, "tau": 0.0, "alpha3": 0.0, "bias": 0.0}
    out_path = ROOT / "out" / "coeffs_v3.json"
    out_path.write_text(json.dumps(coeffs, indent=2))
    print(f"\nSaved coeffs → {out_path}")

    def predict_fn(sim_df, platform):
        cb = predict_factory_v3(platform, coeffs.get(platform, init.get(platform, {})))
        yr = cb(sim_df)
        return pd.DataFrame({"yaw_rate_pred_rads": yr}, index=sim_df.index)

    print("\n=== Full-dataset score V3 ===")
