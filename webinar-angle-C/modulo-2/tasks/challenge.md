# Your task (módulo 1 — bare harness)
...
- **Tools (1):** sí, en `tools/` (tres helpers finos: list_segments, read_csv_head, run_ks).
- **Context-seed (3):** sí, en `CLAUDE.md` (un braindump, no estructurado).
- **Memory/State (2):** **no** — no hay `AGENTS.md`. Lo que recuerdes lo tienes que reconstruir tú en cada turno desde `CLAUDE.md` o desde el código.
- **Planning (4):** **no** — no hay templates RPI ni instrucciones de cómo planear.
- **Verification (5):** **no** — no hay `evals/`. Si tu output es inconsistente, no hay nada que lo detecte automáticamente.
- **Modularity (6):** **no** — no hay `skills/`. No hay recetas pre-empaquetadas para tareas recurrentes.

Esto es deliberado. Estás trabajando en el harness mínimo. Cuando termines, anota en el resumen ejecutivo qué componentes te hicieron falta — eso es load-bearing para el workshop.
