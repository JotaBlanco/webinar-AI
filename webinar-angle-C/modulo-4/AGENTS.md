...
## Skills inventory (Modularity component, 6) — metadata only, load on demand

- `skills/baseline-residual/` — measure baseline lateral residual per Ford platform. Load before any improvement claim.
- `skills/ablation-study/` — controlled additive ablation across variants with per-platform RMSE delta. Load when attributing fidelity gain.
- `skills/yaw-bias-correction/` — concrete domain skill: constant yaw-rate bias detection + correction. The simplest plausible variant A.

**Universal-agent + skills sequencing rule:** don't reach for a sub-agent until you have a reliable skill. If you find yourself doing the same thing twice, crystallise: walk the workflow → write SKILL.md + helper script → re-run → patch.

If the right skill for a sub-task does not exist, author it (NC-18 loop) rather than embedding the recipe inline in AGENTS.md.
