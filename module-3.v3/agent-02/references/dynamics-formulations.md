...
### Post-hoc residual-feature OLS on V1 (agent-02, m3.v3)
Formulation: `yr = a*yr_v1 + b + c*yr_v1*(v*yr_v1)^2 + d * d(delta_road)/dt`
fit by per-platform OLS on the V1 residual (all 3 non-Tesla platforms together,
~3.5M samples). Result: pooled yaw -0.9%, pooled CTE -4.0%. Per-platform CTE on
Mach-E and IONIQ-5 fell mostly because the `b` term killed an existing
signed-mean drift (~22m and ~12m respectively). The cubic-`c` term and the
steering-rate `d` term contribute << 0.1% each above the affine baseline.
Lesson: when V1 leaves a per-platform signed-mean drift on CTE, a single bias
parameter per platform is the highest-leverage fix; doing OLS on a richer
feature set won't lift much more because the affine `a` absorbs almost any
linear-correlated structure.
