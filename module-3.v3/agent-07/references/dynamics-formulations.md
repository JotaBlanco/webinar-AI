...
## Rung 0.5 — V1 with direction-asymmetric steering gain + additive output bias *[shipped by agent-07]*

### Equations

```
delta_raw  = delta_road_rad − δ₀                            (V1's δ₀)
w_left     = 0.5 · (1 + tanh(delta_raw / eps))              eps = 0.005
g_eff      = g_left · w_left + g_right · (1 − w_left)
delta      = delta_raw · g_eff
yr_ss      = v · delta / (L_eff + K_us · v²)                (V1's L_eff, K_us)
yr_lag[i]  = yr_lag[i−1] + α[i] · (yr_ss[i] − yr_lag[i−1])  α = dt/(τ+dt)
yr_out     = yr_lag + b_offset · 1[v > 2]                   (gated additive)
```
...
|---|---|---|
| `g_left`, `g_right` | sign-dependent steering scale | fit per platform (Nelder-Mead) |
| `b_offset` | gated additive output yaw bias (rad/s) | fit + manual regularisation |
| `eps` | smooth-blend width around δ = 0 | held at 0.005 rad |
| `L_eff`, `K_us`, `τ` | inherited from V1 | V1 |

### Implementation notes

- The smooth tanh blend on the sign indicator keeps the gradient differentiable
  and avoids a discontinuity at δ=0. A hard switch is the eps→0 limit.
- **The b_offset fit overfits subsets.** Fitting jointly on 80 segments produced
  a Lightning b_offset that flipped signed bias on the full 175-segment set.
  Lightning's V1 signed bias was already within threshold (+0.00012) so any
  imposed offset is variance. Mitigation: zero b_offset on any platform whose
  V1 signed bias is already at the noise floor; halve fit values to guard
  against subset bias on platforms that *do* benefit.
- Tesla is V0 passthrough (no truth, can't fit). Document and forget.

### When this helps

When V1's per-platform residual decomposition shows **direction asymmetry**
(`turning-left bias` ≠ `turning-right bias` at the regime-bin level). Probe
this with `inspect-residuals` or a quick numpy slice of residuals masked by
`delta_road_rad > 0` vs `< 0` on a sample of segments.

### Failure modes

- Fitting b_offset on a platform whose V1 signed bias is already within
  threshold: introduces overfit variance and degrades full-dataset CTE.
  **Test for this**: re-score on the full dataset, not the fit subset.
- The asymmetry might be route-distribution-induced (e.g. dev set leans
  right-turn-heavy). Cross-validate on left-right-balanced subsets when
  possible.

### Result

Pooled (full dev set): yaw 0.005874 → 0.005805 (-1.2% vs V1);
CTE 56.807 → 54.689 (-3.7% vs V1).

---

## Tried and shelved

### Steering-derivative feedforward `k_dd · d(δ)/dt · clip(v,0,40)/30` *[shelved by agent-07]*

Hypothesised: V1's first-order lag underfits the transient regime (yaw RMSE
0.0164 vs 0.0044 in straight); a steering-derivative feedforward term should
restore the second-order behaviour. Result: <1% improvement on yaw, ~0% on
CTE, and the fitted k_dd was negative on Mach-E (the term was *removing*
over-anticipation, not adding lag-correction — consistent with np.gradient's
phase artefacts rather than real model dynamics). The transient residual is
not addressable by a scalar input-derivative; it needs an actual second-order
dynamic (rung 1).
