...
Non-trivial tasks here are run as **Research → Plan → Implement** across three fresh context windows, with markdown artifacts (`tasks/research.md`, `tasks/plan.md`, `tasks/implement-notes.md`) as the connective tissue. The plan is locked before implementation starts.

If `tasks/RPI_INSTRUCTIONS.md` exists, follow it. The artifacts are the deliverable shape; the conversation is not.
