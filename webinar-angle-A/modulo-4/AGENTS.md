...
## Evals (computational sensors)

A skill's output is only valid if its eval passes. Evals are deterministic, fast, and run inside `code/`-only territory:

- [`evals/lateral_fidelity_eval.py`](evals/lateral_fidelity_eval.py) — parses the `report.md` produced by the `lateral-fidelity-triage` skill and scores it against structural and physical sanity rules. Run it; if it fails, do **not** silently retry — read the failure modes, patch `skills/lateral-fidelity-triage/SKILL.md` to engineer out that mistake (the *ratchet*), and re-run. The skill itself documents this loop.
