# AGENTS.md — webinar-angle-A / module-3 (crystallise)
...
Compute the baseline (V0) RMSE from the existing `yaw_rate_resid_rads` column **as-is**, with no preprocessing. Any preprocessing belongs **inside V1+**, not V0. Folding a fix into V0 hides the upgrade that earns it.
...
3. **Wheel-deg vs road-rad.** Both columns are in the CSV. Wrong column → factor-of-~15 error.
4. **Sign convention is left-positive.** If `corr(δ, ψ̇)` comes out negative, you have a sign error.
5. **Parameters live in `PARAM_BY_PLATFORM`** — look them up.
6. **KS has no slip.** Most of the residual at high `|a_y|` is *expected* and is what an ST upgrade would close.
7. **Single markdown table in `REPORT.md`.** Variant ladder is the *only* markdown table; everything else is bullet lists.

## Skills inventory

Inspect a skill's metadata before deciding to load its body. Never load all skill bodies eagerly.

- [`skills/lateral-fidelity-triage/SKILL.md`](skills/lateral-fidelity-triage/SKILL.md) — procedure for running the variant ladder with strict marginal-RMSE accounting. Imports its own helper module [`skills/lateral-fidelity-triage/triage.py`](skills/lateral-fidelity-triage/triage.py) (KS-stock baseline, Linear-ST upgrade, Cα fit, residual learner). Use this whenever the task is "improve lateral fidelity on a Ford platform".

## References

Domain docs the agent loads on demand — typically referenced from a skill.

- [`references/ks-vs-st.md`](references/ks-vs-st.md) — bounded catalogue of legitimate upgrades the team has already considered (KS recalibration → Linear ST with prior `C_α` → Linear ST with fit `C_α` → residual learner) and the contract-violating ones to avoid.
