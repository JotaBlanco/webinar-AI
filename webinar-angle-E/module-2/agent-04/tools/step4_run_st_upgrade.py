...
def main() -> int:
    ...
    _P_obj = PARAM_BY_PLATFORM[args.platform]
    # Workflow tool was written expecting a dict, but PARAM_BY_PLATFORM yields
    # frozen dataclasses. Wrap with attribute access so the rest of the tool
    # works unmodified. Recorded as a deviation in REPORT.md.
    class _AttrDictView:
        def __init__(self, obj):
            self._obj = obj
        def __getitem__(self, key):
            return getattr(self._obj, key)
    P = _AttrDictView(_P_obj)
