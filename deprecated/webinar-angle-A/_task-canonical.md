# Canonical challenge task (identical for all 4 módulos)

This file is **outside** the four modules on purpose — it is the single source from which `modulo-N/tasks/lateral-fidelity-challenge.md` is copied. The four agents must receive *byte-identical* tasks; only their surrounding substrate differs. Do not edit one without the other three.
