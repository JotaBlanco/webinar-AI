...
- **platform**: all 522 Ford segments with moving-vehicle samples (~1.39 M rows; Mach-E + F-150 Lightning)
...
- result: `True`
- reasoning: Agent scores against the Ford measured IMU yaw-rate channel, naming the specific CAN signal.
- evidence:
...
- result: `True`
- reasoning: Agent explicitly states that v and δ are clamped to measurements while ψ̇ is predicted.
- evidence:
  > Stayed within the speed-known framing — `(v, δ)` clamped to measurements; I only refined how `ψ̇` is computed from them.

### regime-breakdown-present
- result: `False`
- reasoning: No per-regime (straight/cornering/transient) error breakdown is provided; only aggregate and per-platform numbers.
- evidence: _none_

### methodology-consistent
- result: `True`
- reasoning: Same segment definition and metric definition applied across all ladder variants, declared in the headline.
- evidence:
...
- reasoning: Marginal drops (0.12+2.43+0.27=2.82) reconcile exactly with stated total drop of 2.82 mrad/s; |0|/2.82 = 0 < 0.15.
- evidence:
...
- result: `False`
- reasoning: No regression occurred (all three layers improved RMSE) but the report does not include an explicit 'no regressions observed' statement.
- evidence: _none_
