...
- **platform**: 545 Ford segments (Mach-E + F-150 Lightning)
...
- result: `True`
- reasoning: Scores against measured yaw-rate (ψ̇_meas) on Ford segments, with v and δ clamped — yaw-rate is the scored channel and is identified as measured.
- evidence:
...
- result: `True`
- reasoning: Explicitly states v and δ are clamped (measured) while ψ̇ is predicted — clear clamped-vs-predicted statement.
- evidence:
...
- result: `False`
- reasoning: Only platform-level breakdown is provided; no per-regime (straight/cornering/transient) table or chart of the chosen metric.
- evidence:
...
- result: `True`
- reasoning: Same dataset (545 Ford segments) and same metric (yaw-rate RMS residual on test half) used across the ladder variants in the attribution table.
- evidence:
  > **Yaw-rate RMS residual** across all 545 Ford segments (Mach-E + F-150 Lightning), evaluated on held-out test split (50% of each segment)
...
- result: `True`
- value: `0.0016`, threshold_met: `True`
- reasoning: Sum of marginal drops = 0.00614; total drop = 0.01804 - 0.01191 = 0.00613; |0.00614 - 0.00613|/0.00613 ≈ 0.0016, well under 0.15.
- evidence:
...
- result: `None`
- reasoning: not addressed in report — no regression occurred (all steps reduced RMS) and no explicit 'no regressions observed' statement.
- evidence: _none_
