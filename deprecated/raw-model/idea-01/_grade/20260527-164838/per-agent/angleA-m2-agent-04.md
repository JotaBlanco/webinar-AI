...
- result: `True`
- reasoning: Report explicitly names a measured truth channel from Ford CAN data and excludes Tesla due to no truth channel.
- evidence:
...
- result: `True`
- reasoning: Explicit clamped-vs-predicted statement in the Speed-known contract section.
- evidence:
...
- result: `True`
- reasoning: Variant table breaks out RMSE per regime (straight, cornering_steady, cornering_transient).
- evidence:
  > | Variant | RMSE all (rad/s) | Straight | Corner steady | Corner trans. | Marginal Δ |

### methodology-consistent
- result: `True`
- reasoning: Regime mask is declared once and explicitly applied to all variants with the same metric definition.
- evidence:
...
- reasoning: Sum of marginals equals total drop exactly; |0.00536 - 0.00536|/0.00536 = 0.0, well below 0.15 threshold.
- evidence:
  > **Accounting:** sequential marginal decomposition along V0→V4. Sum of marginals = 0.00536, exactly equal to V0 − V4 = 0.00536.

### honest-regression-flagged
- result: `True`
- reasoning: Regression is explicitly reported with quantified worsening and a physical cause explanation.
- evidence:
