...
load-cost: ~140 tokens metadata, ~170 tokens body.
---
...
9. `predict_returns_correct_shape` — calls `predict` on the first `data/sim/segments/FORD_MUSTANG_MACH_E_MK1/**/sim.csv` (alphabetical). Asserts the return is a `pandas.DataFrame` with column `yaw_rate_pred_rads`, an index identical to the input, no NaN in `yaw_rate_pred_rads`, and no NaN in `x_m`/`y_m` if present.
