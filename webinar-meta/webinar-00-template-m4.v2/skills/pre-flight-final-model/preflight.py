...
m4 adds five gates on top of the m3.v2 checks:
...
def _check_models_md(template_root: Path, checks: list[dict], errors: list[str]) -> None:
    ...
    if n < MIN_MODELS_MD_CANDIDATES:
        _add(checks, errors, name, "fail",
             ...
             f">= {MIN_MODELS_MD_CANDIDATES}. Build more candidates via skills/iterate/.")
        return
    diverse = [e for e in entries if e["rung"] in ("1", "2", "3", "orthogonal")]
    if not diverse:
        _add(checks, errors, name, "fail",
             f"MODELS.md has {n} entries but none are tagged rung 1/2/3/orthogonal. "
             "All rung-0 = the m3.v2 collapse failure mode.")
        return
