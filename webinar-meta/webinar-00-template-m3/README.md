...
summary: Module-3 template for the lateral-fidelity webinar. Same eight-skill toolkit and shared math library as m2, plus five short domain-knowledge reference documents (anti-patterns + approach-menu + two-kpi-tradeoff with worked examples and failure-mode indexes; exploration-discipline; ceiling-moves) and an EXPERIMENTS.md log template. The references are the m3 increment over m2 — everything else is identical.
...
- **`exploration-discipline.md`** — protocol for naming ≥3 alternatives before commitment + the `EXPERIMENTS.md` log convention.
- **`ceiling-moves.md`** — four unexplored moves above the current best-known ceiling (multi-seed fold averaging, CTE-aware loss, constrained joint fit, dynamic ST). Sequenced by ROI vs cost. Only loaded after the agent has already beaten V0 by ≥+30% on both KPIs.
