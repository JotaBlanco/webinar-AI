"""dst_steer_compliance/fit.py — fit {C_αf, C_αr, Iz, K_compl, k_ackermann}."""
...
def _load_score_module(template_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("_dst_steer_score", str(path))
    mod = importlib.util.module_from_spec(spec); sys.modules["_dst_steer_score"] = mod
...
def _predict_factory(params: dict):
    ...
    def _predict(sim_df, platform):
        ...
        v = sim_df["v_mps"].to_numpy(); yaw_v0 = sim_df["yaw_rate_pred_rads"].to_numpy()
        ...
        for i in range(1, n):
            ...
            state = _predict_mod._steer_step(state, float(delta[i]), float(v[i]), p_local, dt)
...
def fit_one_platform(platform: str, *, template_root: Path, score_module) -> dict:
    ...
    base["K_compl"] = 1e-5
    base["k_ackermann"] = 0.0
    spec = FitSpec(
        init=[base["C_alpha_f"], base["C_alpha_r"], base["Iz"], base["K_compl"], base["k_ackermann"]],
        bounds=[(20000.0, 250000.0), (20000.0, 250000.0), (1500.0, 8000.0), (1e-7, 1e-4), (-0.5, 0.5)],
        names=["C_alpha_f", "C_alpha_r", "Iz", "K_compl", "k_ackermann"],
    )
...
def fit_all(template_root: Path | None = None) -> dict:
    ...
    out = {"_model": "dst_steer_compliance", "_fitted_from": str(template_root)}
    for platform in PLATFORM_PRIORS:
        print(f"  [dst_steer_compliance] fitting {platform} ...", flush=True)
