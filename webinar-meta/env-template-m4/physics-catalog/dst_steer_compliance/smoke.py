"""dst_steer_compliance smoke — synthetic, sweep delta amplitudes."""
...
def _segment(*, large_delta: bool) -> pd.DataFrame:
    n, dt = 600, 0.01
    t = np.arange(n) * dt
    amp = 0.12 if large_delta else 0.04
...
def main() -> int:
    ...
    for platform in PLATFORM_PRIORS:
        for mode in ("small_delta", "large_delta"):
            seg = _segment(large_delta=(mode == "large_delta"))
            try:
