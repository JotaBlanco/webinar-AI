"""dst_twin_track/fit.py — fit {C_αf, C_αr, Iz, h_cg, track_width, k_LLT_f}."""
...
from _common import (  # noqa: E402
    ...
    V_FLOOR_MPS, discover_dev_segments, find_template_root,
    fit_with_route_cv, write_coeffs,
)
...
def _load_score_module(template_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("_dst_twin_score", str(path))
    mod = importlib.util.module_from_spec(spec); sys.modules["_dst_twin_score"] = mod
...
def _predict_factory(params: dict):
    ...
    def _predict(sim_df, platform):
        ...
        for i in range(1, n):
            ...
            state = _predict_mod._twin_track_step(state, float(delta[i]), float(v[i]), p_local, dt)
...
def fit_one_platform(platform: str, *, template_root: Path, score_module) -> dict:
    ...
    base["track_width"] = 1.6 if "MUSTANG" in platform or "IONIQ" in platform else 1.75
    base["k_LLT_f"] = 0.5
    spec = FitSpec(
        init=[base["C_alpha_f"], base["C_alpha_r"], base["Iz"],
              base["h_cg"], base["track_width"], base["k_LLT_f"]],
        bounds=[(20000.0, 250000.0), (20000.0, 250000.0), (1500.0, 8000.0),
                (0.30, 1.20), (1.3, 2.1), (0.2, 0.8)],
        names=["C_alpha_f", "C_alpha_r", "Iz", "h_cg", "track_width", "k_LLT_f"],
    )
...
def fit_all(template_root: Path | None = None) -> dict:
    ...
    out = {"_model": "dst_twin_track", "_fitted_from": str(template_root)}
    for platform in PLATFORM_PRIORS:
        print(f"  [dst_twin_track] fitting {platform} ...", flush=True)
